...
## Motivation
Currently, there is no standardized way to create derived NFTs. However, with the integration of KIP-155, this proposal enables the NFT's owner to derive new NFTs, which can then be used for renting out within a specific time period.
...
The derived NFT is usable for a predefined time period only. Our approach involves making the derived NFT a short-lived token, designed for renting out to other users for temporary usage. Once the specified time period expires, the derived NFT becomes unusable and must be burnt to reclaim the underlying NFT. However, this does not hinder the derived NFT from being transferred, traded, or utilized, just like all other KIP-17 NFTs.
